...
async def _register_and_login(
    client: AsyncClient, username="testuser", password="testpass123", email="test@test.com"
):
...
async def test_scan_page_requires_login(client: AsyncClient):
    resp = await client.get("/scan", follow_redirects=False)

    assert resp.status_code == 302
    assert resp.headers["location"] == "/login"


async def test_scan_page_and_api_accept_web_cookie(client: AsyncClient):
    await _register_and_login(client, username="scanuser", email="scanuser@test.com")

    login_resp = await client.post(
        "/login",
        data={"username": "scanuser", "password": "testpass123"},
        follow_redirects=False,
    )
    assert login_resp.status_code == 302

    scan_resp = await client.get("/scan")
    ...
    assert me_resp.json()["username"] == "scanuser"

    ingest_resp = await client.post(
        "/api/v1/ingest",
        json={
            "shelf": {"shelf_id": "SCAN1", "row": "Z", "position": 9, "height": 2},
            "source_type": "isbn",
            "isbn": "9780140449136",
            "title": "Cookie Powered Scan",
        },
    )
    assert ingest_resp.status_code == 201
    assert ingest_resp.json()["title"] == "Cookie Powered Scan"
