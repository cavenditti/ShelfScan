...
async def scan_page(
    ...
    return tpl.TemplateResponse(request, "scan.html", {"user": user})
...
async def register_submit(
    ...
    if existing.first():
        return tpl.TemplateResponse(
            request, "register.html", {"error": "Username or email already taken"}
        )
...
async def books_page(
    ...
    books = list(
        (
            await session.exec(
                stmt.order_by(Book.created_at.desc()).offset((page - 1) * per_page).limit(per_page)
            )
        ).all()
    )
...
async def book_create_page(
    ...
    if not user:
        return RedirectResponse("/login", status_code=302)
    return tpl.TemplateResponse(
        request, "book_form.html", {"user": user, "book": None, "error": None}
    )
        ...
        try:
            ...
            return tpl.TemplateResponse(
                ...
                "book_form.html",
                {
                    "user": user,
                    "book": None,
                    "error": "Extra metadata must be a valid JSON object.",
                },
            )
...
async def book_edit_page(
    ...
    if not book:
        return RedirectResponse("/books", status_code=302)
    return tpl.TemplateResponse(
        request, "book_form.html", {"user": user, "book": book, "error": None}
    )
...
async def shelves_page(
    ...
    shelves_list = list(
        (
            await session.exec(
                select(Shelf).order_by(Shelf.shelf_id, Shelf.row, Shelf.position, Shelf.height)
            )
        ).all()
    )
    ...
    for s in shelves_list:
        count = (
            await session.exec(select(func.count(BookCopy.id)).where(BookCopy.shelf_id == s.id))
        ).one()
    ...
    return tpl.TemplateResponse(
        ...
        "shelves.html",
        {
            ...
            "shelf_copy_counts": shelf_copy_counts,
        },
    )
...
async def shelf_create_page(
    ...
    if not user:
        return RedirectResponse("/login", status_code=302)
    return tpl.TemplateResponse(
        request, "shelf_form.html", {"user": user, "shelf": None, "error": None}
    )
...
async def shelf_create_submit(
    ...
    existing_shelf_id = (
        await session.exec(select(Shelf).where(Shelf.shelf_id == form_shelf["shelf_id"]))
    ).first()
...
async def shelf_detail_page(
    ...
    copies = list(
        (await session.exec(select(BookCopy).where(BookCopy.shelf_id == shelf_db_id))).all()
    )
    books = {}
    for c in copies:
...
async def shelf_edit_page(
    ...
    if not shelf:
        return RedirectResponse("/shelves", status_code=302)
    return tpl.TemplateResponse(
        request, "shelf_form.html", {"user": user, "shelf": shelf, "error": None}
    )
...
async def shelf_delete(
    ...
    copy_count = (
        await session.exec(select(func.count(BookCopy.id)).where(BookCopy.shelf_id == shelf_db_id))
    ).one()
    if copy_count > 0:
        return RedirectResponse(
            f"/shelves/{shelf_db_id}?error=Cannot+delete+shelf+with+copies", status_code=302
        )
    ...
    for row in [r.strip() for r in rows.split(",") if r.strip()]:
        for pos in range(pos_from, pos_to + 1):
            for h in range(height_from, height_to + 1):
                ...
                if not existing:
                    existing_shelf_id = (
                        await session.exec(select(Shelf).where(Shelf.shelf_id == coord_label))
                    ).first()
                    if existing_shelf_id:
                        return HTMLResponse(
                            content=(
...
async def admin_user_create(
    ...
    if existing:
        return RedirectResponse(
            "/admin/users?error=Username+or+email+already+taken", status_code=302
        )
    ...
    await session.commit()
    return RedirectResponse(
        f"/admin/users?success=User+{username.strip()}+created", status_code=302
    )
